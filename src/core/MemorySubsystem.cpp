...
void TokenBucket::refill() {
    ...
    // 关键修复：用 cap 前的原始值更新 lastRefillTokens，避免累积误差
    double raw_tokens = lastRefillTokens + elapsed_tokens;
    lastRefillTokens = raw_tokens;  // 先更新基准（cap 前）
    tokens = std::min(maxTokens, raw_tokens);  // 再 cap
